...
const inputClass =
  'w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm outline-none transition placeholder:text-gray-400 focus:border-orange-600 focus:bg-white focus:ring-4 focus:ring-orange-100';

export default function DiscontinueMedicationDialog({
  medication,
  onClose,
  onConfirm,
      ...
      <div className="w-full max-w-md overflow-hidden rounded-[2rem] bg-[#faf9f6] shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-5 sm:p-6">
          <p className="mb-5 text-sm text-gray-600">
            Are you sure you want to discontinue <strong>{medication.name}</strong>? Please select the date of discontinuation.
          </p>

          <div className="space-y-2">
            <label htmlFor="discontinueDate" className="block text-sm font-medium text-gray-700">
              Discontinuation Date
            </label>
            <input id="discontinueDate" type="date" required value={date} onChange={(e) => setDate(e.target.value)} className={inputClass} />
          </div>
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:justify-end">
            ...
            <button type="submit" className="rounded-full bg-orange-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-orange-700">Confirm</button>
          </div>
        </form>
      </div>
    </div>
